...
def defineModel(model_type):
    ...
    if model_type == 'mynet2_50':
        model_ft = fvrnet.mynet2(layers=[3, 4, 6, 3])
    elif model_type == 'mynet2_101' or model_type == 'test':
        model_ft = fvrnet.mynet2(layers=[3, 4, 23, 3])
    elif model_type == 'mynet2_150' or 'mynet2_150_l1':
        model_ft = fvrnet.mynet2(layers=[3, 8, 36, 3])
    elif model_type == 'mynet4_150':
        model_ft = fvrnet.mynet4()
    else:
...
class FreehandUS4D(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        case_recon_vol_fn = path.join(recon_dir, case_id, '{}.dcm'.format(case_id))
